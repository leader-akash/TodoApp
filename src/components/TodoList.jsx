...
//     setSelectedTasks((prevSelected) => prevSelected.includes(id) ? prevSelected.filter((taskId) => taskId !== id)
//         : [...prevSelected, id]
//     );
//   };
...
import { useSelector, useDispatch } from 'react-redux';
import { completeTask, deleteTask } from '../redux/todoSlice';
import EditTaskModal from './editTodo';
...
const TodoList = ({ filter }) => {
  ...
  return (
    <div>
      ...
      {editingTask && (
        <EditTaskModal task={editingTask} onClose={handleCloseModal} />
      )}
    </div>
  );
};
